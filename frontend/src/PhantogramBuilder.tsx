...
const loadSettings = (): Settings => {
    try {
        return { ...defaults, ...JSON.parse(localStorage.getItem('aaf-phantogram-settings') || '{}') }
    } catch {
        return defaults
    }
}
...
function PhantogramBuilder({ isDepthMapReady, setProcessingStage }: Props) {
    ...
    const params = useMemo(() => new URLSearchParams({
        ...
        width_in: String(settings.widthIn),
        height_in: String(settings.heightIn),
        view_distance_in: String(settings.viewDistanceIn),
        eye_height_in: String(settings.eyeHeightIn),
        ipd_mm: String(settings.ipdMm),
        relief_mm: String(settings.reliefMm),
    ...
    useEffect(() => {
        ...
        const timer = window.setTimeout(async () => {
            ...
            try {
                const response = await fetch(`${apiUrl}/special/phantogram?scope=preview&${params.toString()}`, {
                ...
                setError('Could not render the phantogram preview.')
    ...
    }, [apiUrl, isDepthMapReady, params, setProcessingStage])

    const patch = (values: Partial<Settings>) => setSettings(current => ({ ...current, ...values }))

    const setPreset = (value: string) => {
        if (value === '8x6') patch({ widthIn: 8, heightIn: 6 })
        if (value === '10x7.5') patch({ widthIn: 10, heightIn: 7.5 })
        if (value === '7x5') patch({ widthIn: 7, heightIn: 5 })
    }
    ...
    const download = async () => {
        ...
        try {
            ...
            link.href = url
            link.download = `phantogram-${settings.glasses}-${settings.widthIn}x${settings.heightIn}in.png`
            ...
            console.error(caught)
            setError('Could not create the full-resolution phantogram.')
            setProcessingStage('error')
        } finally {
    ...
    return <main className="phantogramWorkspace">
        <section className="phantogramIntro">
            <div>
                <div className="panelLabel">PHYSICAL PRINT</div>
                <h2>AI Relief Phantogram</h2>
                <p>Uses the current source image and active depth map from 3D Studio as a physical height field above a flat print. Each eye is projected independently onto the print plane, then combined as an anaglyph.</p>
            </div>
            <div className="phantogramBadge">EXPERIMENTAL · GEOMETRIC</div>
        </section>

        {!isDepthMapReady && <div className="phantogramNotice">
            <strong>No current source + depth map.</strong>
            <span>Load an image in 3D Studio first. Phantogram uses that same source and whichever AI or imported depth map is active.</span>
        </div>}
        ...
        <div className="phantogramGrid">
            <section className="phantogramControls">
                ...
                <div className="phantogramFields">
                    ...
                    <label><span>Eye separation</span><input type="number" min="45" max="80" step="0.5" value={settings.ipdMm} onChange={e => patch({ ipdMm: Number(e.target.value) || 63 })}/><small>mm</small></label>
                    <label><span>Maximum apparent relief</span><input type="number" min="0" max="100" step="1" value={settings.reliefMm} onChange={e => patch({ reliefMm: Number(e.target.value) || 0 })}/><small>mm</small></label>
                </div>

                <label className="phantogramToggle"><input type="checkbox" checked={settings.reverseDepth} onChange={e => patch({ reverseDepth: e.target.checked })}/><span><strong>Reverse depth</strong><small>Use if the result appears carved into the page instead of rising from it.</small></span></label>

                <div className="phantogramPrintCheck">
                    <div><strong>Print calibration</strong><span>Print the ruler first and confirm the 100 mm marks physically measure 100 mm.</span></div>
                    <a href={`${apiUrl}/phantogram/calibration?dpi=${settings.dpi}`}>Download 100 mm ruler</a>
                </div>
            </section>
            ...
            <section className="phantogramPreviewCard">
                ...
                <div className="phantogramViewDiagram" aria-label="Viewing geometry diagram">
                    <div className="eyes">● ●</div><div className="sightLines">╲ ╱</div><div className="paperLine"/><span>Lay print flat · near edge toward you</span>
                </div>
                {error && <div className="phantogramError">{error}</div>}
                <button className="phantogramDownload" disabled={!isDepthMapReady || downloading} onClick={() => void download()}>{downloading ? 'Preparing full-resolution print…' : 'Download print-ready PNG'}</button>
                <p className="phantogramFinePrint">The PNG contains physical DPI metadata. Print at <strong>100% / Actual Size</strong>, never Fit to Page. The source and depth map are center-cropped together to the selected print aspect ratio.</p>
            </section>
        </div>
    </main>
}
